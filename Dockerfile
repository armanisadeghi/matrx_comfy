...
# Set working directory to match external project root
WORKDIR /matrx_comfy
...
# Copy the workflows directory into the container without overwriting existing files
COPY workflows /matrx_comfy/ComfyUI/user/default/workflows/

# Ensure the workflows directory exists and copy files only if they don't already exist
RUN mkdir -p /matrx_comfy/ComfyUI/user/default/workflows && \
    cp -n workflows/* /matrx_comfy/ComfyUI/user/default/workflows/ || true
...
# Run ComfyUI on port 8189 with full startup options
